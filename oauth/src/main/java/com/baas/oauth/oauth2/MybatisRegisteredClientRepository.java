...
public class MybatisRegisteredClientRepository implements RegisteredClientRepository {
    ...
    public void save(RegisteredClient registeredClient) {
        ...
        Register dto = toDto(registeredClient);
        Optional<Register> existing = this.registerMapper.findById(dto.getId());

        if (existing.isPresent()) {
            registerMapper.update(dto);
        } else {
            registerMapper.save(dto);
        }
    }

    @Override
    public RegisteredClient findById(String id) {
        return registerMapper.findById(id).map(this::toSpringObject).orElse(null);
    }

    @Override
    public RegisteredClient findByClientId(String clientId) {
        return registerMapper.findByClientId(clientId).map(this::toSpringObject).orElse(null);
    }

    private RegisteredClient toSpringObject(Register dto) {
    ...
    private Register toDto(RegisteredClient registeredClient) { // 9. [수정] toEntity -> toDto, RegisterEntity -> Register
        ...
        Register dto = new Register(); // 10. [수정] RegisterEntity -> Register
